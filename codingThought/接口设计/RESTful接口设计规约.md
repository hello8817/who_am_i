...
明确你的GET和POST方法。当你用浏览器去访问不同页面的时候，这两个是最常见的请求方法。   
这里至少有四个半非常重要的HTTP需要了解。我之所以说“半个”的意思是PATCH这个动词非常类似于PUT，并且它们俩也常常被开发者绑定到同一个API上。  
...
404 NOT FOUND – [所有接口请求] 客户端引用了一个不存在的资源或集合，服务器什么也没做
500 INTERNAL SERVER ERROR – [所有接口请求] 服务器发生内部错误，客户端无法得知结果，即便请求已经处理成功
...
当使用不同的HTTP请求方法向服务器请求时，客户端需要在返回结果里面拿到一系列的信息。下面的列表是非常经典的RESTful API定义：
...
目前，大多数“让人愉悦”的API都为RESTful接口提供JSON数据。诸如Facebook，Twitter，Github等等你所知的。XML曾经也火过一把（通常在一个大企业级环境下）。这要感谢SOAP，不过它已经挂了，并且我们也没看到太多的API把HTML作为结果返回给客户端（除非你在构建一个爬虫程序）。
...
- [Restful API 设计参考原则](https://www.cnblogs.com/WayneZeng/p/7591465.html)
- [如何设计好的RESTful API 之好的RESTful API 特征](https://blog.csdn.net/ywk253100/article/details/25654021)
